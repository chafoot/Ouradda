...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('♥️ Love Our Adda ♥️')
            else:
                await query.message.edit_text(
                    "I'm not connected to any groups!\nCheck /connections or connect to any groups",
                    quote=True
                )
                return await query.answer('♥️ Thank You Our Adda ♥️')
        ...
        if groupids is None:
            ...
            return await query.answer('♥️ Thank You Our Addar ♥️')
        buttons = []
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Please join Updates Chaneel then click on 🔄 Try Again", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        await query.answer('♥️ Thank You Our Adda ♥️')
        ...
        await query.answer("Thank You Our Adda")
        ...
        try:
            ...
            mybad = await query.message.reply_text(
           ...
           #my = await query.message.reply_text(
        ...
        try:
            ...
            reply_markup = InlineKeyboardMarkup(btn)
            await client.send_message(int(user_id), f"😒 oops! {from_user.mention}, Your requested content named **`{movie}`** is not available right now, we are really trying our best to serve you this content, \n\n❤ Thank you for your passion❤", reply_markup=reply_markup)
        ...
        try:
            btn = [[
                InlineKeyboardButton(text=f"🅶 Google", url=f"https://www.google.com/search?q={movie}")
            ],[
                InlineKeyboardButton(text=f"🅱 Bing", url=f"https://www.bing.com/search?q={movie}")
            ]]
            btn_lzdv = [
                [
                InlineKeyboardButton(text=f"🗑 Delete Log ❌", callback_data = "close_data")
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 100:
            ...
            if not files:
                ...
                l = await message.reply_text(text=f"△ 𝙷𝚎𝚢 `<a href='https://t.me/ourdeals1'>{message.from_user.first_name}</a>` 😎,\n\nʏᴏᴜʀ ʀᴇQᴜᴇꜱᴛ ʜᴀꜱ ʙᴇᴇɴ ꜱᴇɴᴛ ᴛᴏ ᴏᴜʀ **ᴀᴅᴍɪɴ'ꜱ ᴅᴀꜱʜʙᴏᴀʀᴅ** !\nᴘʟᴇᴀꜱᴇ ᴋᴇᴇᴘ ꜱᴏᴍᴇ ᴘᴀᴛɪᴇɴᴄᴇ !\nᴛʜᴇʏ ᴡɪʟʟ ᴜᴘʟᴏᴀᴅ ɪᴛ ᴀꜱ ꜱᴏᴏɴ ᴀꜱ ᴘᴏꜱꜱɪʙʟᴇ.\n\n📝𝘾𝙤𝙣𝙩𝙚𝙣𝙩 𝙣𝙖𝙢𝙚 : `{search}`\n👮𝙍𝙚𝙦𝙪𝙚𝙨𝙩𝙚𝙙 𝘽𝙮 : `{message.from_user.first_name}`",
                                                                                                       reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("━ • │▌║  Add me to your Group  ║▌│ • ━", url=f'http://t.me/{temp.U_NAME}?startgroup=true')], [InlineKeyboardButton(text=f"🅶 Google", url=f"https://www.google.com/search?q={movie}"), InlineKeyboardButton(text=f"🅱 Bing", url=f"https://www.bing.com/search?q={movie}")]]))
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ to ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/ouraddaa/9'),
        ] 
    )
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("Hey <a href='https://t.me/ourdeals1'>Sona ♥️</a>, Sᴘᴇʟʟɪɴɢ Mɪꜱᴛᴀᴋᴇ Bʀᴏ ‼️\nᴅᴏɴ'ᴛ ᴡᴏʀʀʏ 😊 Cʜᴏᴏꜱᴇ ᴛʜᴇ ᴄᴏʀʀᴇᴄᴛ ᴏɴᴇ ʙᴇʟᴏᴡ 👇 ", disable_web_page_preview=True)
        await asyncio.sleep(10)
        await k.delete()
        return
    ...
    await msg.reply("Hey <a href='https://t.me/ourdeals1'>Sona ♥️</a>, Sᴘᴇʟʟɪɴɢ Mɪꜱᴛᴀᴋᴇ Bʀᴏ ‼️\nᴅᴏɴ'ᴛ ᴡᴏʀʀʏ 😊 Cʜᴏᴏꜱᴇ ᴛʜᴇ ᴄᴏʀʀᴇᴄᴛ ᴏɴᴇ ʙᴇʟᴏᴡ 👇 ", disable_web_page_preview=True,
                    reply_markup=InlineKeyboardMarkup(btn))
