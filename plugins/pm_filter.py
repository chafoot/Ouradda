...
async def doc(bot, update):
    try:
        ...
        await ms.edit("三 𝘗𝘳𝘦𝘱𝘢𝘳𝘪𝘯𝘨 𝘵𝘰 𝘳𝘦𝘤𝘦𝘪𝘷𝘦 Our 𝘧𝘪𝘭𝘦...︻デ═一")
        c_time = time.time() 
        try:
           if type == "document":
              await bot.send_document(
	            update.message.chat.id,
                       ...
                       caption=caption, 
                       progress=progress_for_pyrogram,
                       progress_args=( "**⎝⎝✧ ʀᴇᴄɪᴇᴠɪɴɢ ꜰɪʟᴇ ꜰʀᴏᴍ our ꜱᴇʀᴠᴇʀ ✧⎠⎠**",  ms, c_time))
           elif type == "video": 
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            reply_markup = InlineKeyboardMarkup(btn)
            await client.send_message(int(user_id), f"😒 oops! Dear, Your requested content named `{movie}` is not available right now, we are really trying our best to serve you this content Please wait will upload ASAP\n\n❤ Thank You for the contribution", reply_markup=reply_markup)
        ...
        try:
            ...
            reply_markup_lzdv = InlineKeyboardMarkup(btn_lzdv) 
            reply_markup = InlineKeyboardMarkup(btn)
            await client.send_message(int(user_id), f"🌍 Heads up! Your spelling matters.\n\n We noticed a typo in your search for `{movie}`./n/nNo worries, we have `{movie}` in our library! Use one of the below buttons to double-check the spelling and try searching again.\n\n❤Thank u for supporting us.", reply_markup=reply_markup)
    ...
    #Adding This feature to the bot to get the controls over the groups  
        ...
        try:
            ...
            ms.edit(f"Got a error Bro:\n{e}" )
            logger.error(f"Please solve this Error Our Addaa Bro : {e}")
        ...
        try:
            ...
            ms.edit(f"Got a error Our Addaa:\n{e}" )
            logger.error(f"Please solve this Error Our Addaa Bro : {e}")
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("Hey Sona! The requested content is currently unavailable in our database, have some patience 🙂 - our great admin will upload it as soon as possible ♥️ ")
        await asyncio.sleep(10)
        await k.delete()
        return
    ...
    await msg.reply("Hey sona, did you checked your spelling properly, here are some suggestions for you, please check if your requested content match anyone of these following suggestions... ♥️ ",
                    reply_markup=InlineKeyboardMarkup(btn))
